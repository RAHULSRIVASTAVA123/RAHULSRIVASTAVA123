...
def fetch_stats():
    stats = {"repos": 5, "stars": 0, "commits": 20, "followers": 1,
             "prs": 0, "issues": 0}
    try:
...
def main():
    stats = fetch_stats()
    handle = "rahul_.srivastava"

    info = []
    info.append(f'<tspan x="413" y="30">{esc(handle)}</tspan> -{"—" * (WIDTH - len(handle) - 8)}-—-')
    info.append(kv(50, ["OS"], "Full Stack Engineer"))
    info.append(kv(70, ["Uptime"], uptime_string()))
    info.append(kv(90, ["Role"], "Software Engineer @ SyneGrow (Backend)"))
    info.append(kv(110, ["Company"], "SyneGrow | ex-Northern Arc Capital"))
    info.append(kv(130, ["Education"], "B.E. CSE - Chitkara University (2025)"))
    info.append(kv(150, ["IDE"], "IntelliJ IDEA, VS Code"))
    info.append(blank(170))
    info.append(kv(190, ["Languages", "Programming"], "Java, JS, TS, Python, SQL"))
    info.append(kv(210, ["Languages", "Web"], "HTML, CSS, JSON, YAML"))
    info.append(kv(230, ["Languages", "Real"], "English, Hindi"))
    info.append(blank(250))
    info.append(kv(270, ["Frameworks"], "Spring Boot, NestJS, React, Angular"))
    info.append(kv(290, ["Cloud", "DevOps"], "AWS, Docker, CI/CD, HERE Maps"))
    info.append(kv(310, ["Databases"], "PostgreSQL, MySQL, MongoDB"))
    info.append(header(330, "- Contact"))
    info.append(kv(350, ["Email", "Personal"], "writetorahul3125@gmail.com"))
    info.append(kv(370, ["LinkedIn"], "linkedin.com/in/rahul-srivastava-47399b258"))
    info.append(kv(390, ["Location"], "Bengaluru, Karnataka, India"))
    info.append(kv(410, ["GitHub"], "RAHULSRIVASTAVA123"))
    ...
    info.append(kv(510, ["Experience"], "1.5+ Yrs | Fintech, Logistics, Non-Profit"))
